...
def create_base_dirs(pkudir,test_cases,where_test_starts,workdir,datadir,verbose,class_scenarios,labeldir,noval):
  """
  ----------------------------------------------------------
  ...
  #in case you need to create source and target directories where each frame appears only once
  labels = open(labeldir +"labelsM.csv")
  ...
  while line!="":
    ...
    label_action_num = int(values[0]) #action class
    label_actionFile_num = str(values[3][0:len(values[3])-1]) #action file to which our action belongs 0002-0364 for the PKU
    while len(label_actionFile_num)!=4:
      label_actionFile_num = "0" + label_actionFile_num

    temp = [label_action_num,label_actionFile_num]

    if count_entries%2==r2:
      list_for_source.append(temp)
    else: list_for_target.append(temp)

    count_entries+=1
    line = labels.readline()


  #print (list_for_target,"\n dsfdsfs \n",list_for_source)
      ...
      for camera_case in test_cases[case]:
        ...
        for image in dirI:
          ...
          if (action_num in processed_actions) or (int(action_num) in processed_actions):
            ...
            if (camera_case not in target) and (camera_case in source) and  ([int(action_num),str(actionFile_num)] in list_for_source[0:len(list_for_source)]): #for the training and validation sets

              if noval==True:
                  max_val_percentage = 0
              else:
                  max_val_percentage = 0.15

            

              if noval == True and validation_size / entries < max_val_percentage : #using 10% of the data for validation
            ...
            elif (camera_case in target) and (camera_case not in source) and ([int(action_num),str(actionFile_num)] in list_for_target[0:len(list_for_target)]): #for the testing set
...
parser.add_argument("-train","--training",default="False", help="if true then model percedes to training",
                    action="store_true")
parser.add_argument("-noval","--novalidation",default="False", help="if true then model percedes to training",
                    action="store_true")
...
if os.path.exists(args.pkudir) == False:
  if args.verbose == True: print("the base directory for source and target will be created")
...
if args.directories==True : create_base_dirs(args.pkudir,test_cases,where_test_starts,args.workdir + "/",args.datadir +"/",args.verbose,{11,51},args.labeldir +"/",args.novalidation)
if args.training==True: trm.train(args.pkudir+"/",3,"model_new_",test_cases,where_test_starts,args.workdir + "/",args.datadir + "/",args.verbose,{11,51},args.novalidation)
